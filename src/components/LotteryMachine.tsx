...
function LotteryMachine({ members, onDrawComplete, drawRequested, onDrawStart }: LotteryMachineProps) {
  ...
  const rainbowParticlesRef = useRef<EasterEggParticle[]>([])
  const activeEggRef = useRef<EasterEggDef | null>(null)
  ...
  useEffect(() => {
    ...
    Events.on(engine, 'collisionStart', (event) => {
      ...
      for (const pair of event.pairs) {
        ...
        // Secret Easter egg mode — check registry for this winner's name
        const matchingEgg = EASTER_EGGS.find(e => e.matches(ballBody.label))
        activeEggRef.current = matchingEgg ?? null
        if (matchingEgg) {
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      if (anim) {
        ...
        // Easter egg animation for this winner
        if (secretModeRef.current && elapsed > MOVE_MS && activeEggRef.current) {
          const secretElapsed = performance.now() - secretStartRef.current - MOVE_MS
          activeEggRef.current.render({
            ctx,
            elapsed: secretElapsed,
            particles: rainbowParticlesRef.current,
